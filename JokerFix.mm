...
static void loadPrefs(void) {
    ...
    NSDictionary *p = CFBridgingRelease(CFPreferencesCopyMultiple(
        NULL, dom, kCFPreferencesCurrentUser, kCFPreferencesAnyHost));
    if (!p) return;
    id v;
    if ((v = p[@"Enabled"]))     gEnabled   = [v boolValue];
    if ((v = p[@"ReplaceImage"])) gReplacePath = [v isKindOfClass:NSString.class] ? v : nil;
    if ((v = p[@"Scale"]))       gScale     = [v floatValue];
    if ((v = p[@"Quality"]))     gQuality   = [v floatValue];
    if ((v = p[@"StripMeta"]))   gStripMeta = [v boolValue];
}
...
static UIImage *redraw(UIImage *img, CGFloat scale) {
    ...
    [img drawInRect:(CGRect){ CGPointZero, s }];
